...
EventEmitter.prototype.off = function (type, listener, context) {
    ...
    if (isListener(listener)) {
        if (removeListener(listeners, listener, context && context === this ? null : context)) {
...
EventEmitter.prototype.removeAllListeners = function (type, context) {
    ...
    var remover = hasContext ? removeAllListeners : removeAllListenersWithContext;
        ...
        if (hasNotType) {
            for (key in _events) {
                if (key !== 'removeListener' && _events.hasOwnProperty(key)) {
                    remover(this, key, _events);
                }
            }

            remover(this, 'removeListener', _events);
        } else {
            remover(this, type, _events);
...
function removeListener(listeners, listener, context) {
    ...
    while (index--) {
        ...
        if (event.callback === listener && event.context === context) {
            break;
        }
    }
